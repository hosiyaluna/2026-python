...
def get_top_counties(all_years: dict[int, dict[str, int]], top_n: int = 10) -> list[str]:
    ...
    total_counts.pop("其他", None)
    ranked = sorted(total_counts.items(), key=lambda item: (-item[1], item[0]))
    return [county for county, _ in ranked[:top_n]]


def build_heatmap(years: list[int] | None = None, top_n: int = 10) -> Path:
    """產生縣市 × 年份熱力圖並存檔。"""
    years = years or [109, 110, 111, 112, 113, 114]
    ...
    ax.set_xticklabels([f"{year}學年度" for year in years])
    ax.set_yticks(range(len(counties)))
    ax.set_yticklabels(counties)
    ax.set_title("109-114學年度各縣市招生人數熱力圖")

    for row_index, county in enumerate(counties):
        for col_index, year in enumerate(years):
            ax.text(col_index, row_index, str(all_years[year].get(county, 0)), ha="center", va="center", color="black", fontsize=9)

    fig.colorbar(image, ax=ax, label="人數")
    fig.tight_layout()
